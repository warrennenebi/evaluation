...
@section('content')
    <form id="demande_bien_service">
        ...
        <div class="form-group" id="commune" style="margin-top: 15px;">
            <label for="commune" style="font-weight: bold; color: #000000;">Commune</label><br>
            <select id="select1" class="form-control form-control-sm dataOptions" name="commune_id" style="width: 460px;" required>
                <option disabled>--- Choisissez la commune concerné ---</option>
                @foreach($communes as $commune)
                        <option
                        @if($selectreseau->first()->commune_id == $commune->id)
                            @selected(true)
                        @endif
                        value="{{ $commune->id}}">{{ $commune->label}}
                        </option>
                @endforeach
            </select>
        </div>
        <div class="form-group" style="margin-top: 15px;">
            ...
            <input class="form-control form-control-sm" type="text" name="nom_centre" style="width: 460px;"  value="{{$nom_centre}}" required>
            <div class="form-group" style="margin-top: 15px;">
                        ...
                        <div class="col-md-6 pr-0">
                            <div class="form-check">
                                <label class="form-check-label">
                                    ...
                                    @if($selectreseau->first()->type->contains($type->id))
                                        @checked(true)
                                    @endif>
                                    {{ $type->label}}
                                </label>
        ...
        <div class="form-group" style="margin-top: 15px;">
            ...
            <input class="form-control form-control-sm" type="text" name="localisation" style="width: 460px;" value="{{$localisation}}" required>
        </div>
        <div class="form-group" style="margin-top: 15px;">
            <label for="contact" style="font-weight: bold; color: #000000;">Contacts</label><br>
            <input class="form-control form-control-sm" type="text" name="contact" style="width: 460px;" value="{{$contact}}" required>
        </div>
    </form>
@stop
...
@section('modalbutton')
    ...
    <button type="button" data-lien="{{route('nouvelle_clinique.update', [$id])}}" data-form="demande_bien_service"  class="btn btn-primary Updatedemande" style="border-radius: 20px; background-color: #21301a; color: #ffffff;">Valider</button>
@stop

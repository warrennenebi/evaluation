...
@section('NomModal', "modalcircuitorgane")
...
@section('content')
    <form id="circuit_organe_validateur" method="POST">
        <div class="form-group" style="margin-top: 15px;">
            ...
            <select name="circuit_organe_id" id="circuit_organe_id" class="form-control form-control-sm dataOptions" style="width: 460px;">
                ...
                @foreach ($type as $circuit)
                    <option value="{{ $circuit->id}}">{{ $circuit->direction->label}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            <input type="text" class="form-control form-control-sm d-none" name="organe_validateur_id" value="{{$organe_validateur_id}}" readonly>
          </div>
    </form>
@stop
...
@section('modalbutton')
    ...
    <button type="button" data-lien="{{route('circuit_organe_validateur')}}" data-form="circuit_organe_validateur" class="btn btn-primary envoie_demande" style="border-radius: 20px; background-color: #21301a; color: #ffffff;">Valider</button>
@stop
